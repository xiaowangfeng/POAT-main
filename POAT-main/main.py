...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mae_error = validate(val_loader, model, criterion, normalizer, cuda=args.cuda, csv_name='test_results.csv')
    ...
    print(f"permute 执行时间: {m} 秒")
    # test best model
    print('---------Evaluate Model on Test Set---------------')
    best_checkpoint = torch.load('model_best.pth.tar')
    model.load_state_dict(best_checkpoint['state_dict'])
    res = validate(test_loader, model, criterion, normalizer, test=True, csv_name='test_results.csv')
